...
function seedData() {
  db.get('SELECT COUNT(*) as count FROM user_accounts', (err, row: any) => {
    if (err) return;
    if (row && row.count === 0) {
      ...
      for (const acc of accounts) {
        db.run(
          `INSERT INTO user_accounts (id, name, email, password, role, permissions) VALUES (?, ?, ?, ?, ?, ?)`,
          [acc.id, acc.name, acc.email, acc.password, acc.role, acc.permissions]
        );
      }
      ...
      db.run(
        `INSERT INTO store_settings (key, value) VALUES ('settings', ?)`,
      ...
      for (const s of suppliers) {
        db.run(
          `INSERT INTO suppliers (id, name, company, phone, email, address, balanceOwed) VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [s.id, s.name, s.company, s.phone, s.email, s.address, s.balanceOwed]
        );
      }
      ...
      for (const c of customers) {
        db.run(
          `INSERT INTO customers (id, name, phone, email, address, balanceReceivable) VALUES (?, ?, ?, ?, ?, ?)`,
          [c.id, c.name, c.phone, c.email, c.address, c.balanceReceivable]
        );
      }
      ...
      for (const p of products) {
        db.run(
          `INSERT INTO products (id, barcode, name, company, category, purchasePrice, retailPrice, wholesalePrice, stock, minStockAlert, unitOfSale) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [p.id, p.barcode, p.name, p.company, p.category, p.purchasePrice, p.retailPrice, p.wholesalePrice, p.stock, p.minStockAlert, p.unitOfSale]
        );
      }
